...
// To display the "Professional Interests" field, add 'true', for example
// show_directory('fac1', 'Professors', true);

// To exclude a faculty type, add it at the end:
// show_directory('', '', '', 'stu');

function show_directory($fac_type = '', $title = '', $interests = 'hide', $exclude = '') {
    $args = array(
            'post_status' => 'publish',
            'post_type' => 'directory',
    ...
    if ( $fac_type == $current_type || ( $fac_type == '' && $current_type !== $exclude ) ) {
        ?>
        <tr>
            <td class="dirImg">
            <?php 
